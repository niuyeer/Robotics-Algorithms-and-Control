...
//followLine && Pleave
   if( ((center.read() > LINETHRESHOLD)||(right.read() > LINETHRESHOLD)||(left.read() > LINETHRESHOLD))&&((abs((postion.getX()+postion.getY()))<5)&& (abs(abs(getAngle(postion,goal)-postion.getZ())-getAngle(start,goal))<0.03) &&(distance(pHit,goal)-distance(postion,goal)<5)) )
    {
//     Serial.print("Start 3 White To Goal");  // tab characte
//     Serial.print("\t");  // tab character
 //     Serial.print((lCount)/(2*192)*(2.56*PI));  // tab character
...
//when it hit obtacle(LINE) then follow Line(turn around obstacle)
  ...
  {
    //hit
    if(buffLeftSensor<LINETHRESHOLD && buffRightSensor<LINETHRESHOLD)
    {
      pHit=postion;
     turnLeft();
     }
